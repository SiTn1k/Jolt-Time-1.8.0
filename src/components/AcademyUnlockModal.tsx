...
export function AcademyUnlockModal({ isOpen, onClose }: AcademyUnlockModalProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          className="fixed inset-0 z-[100] flex items-start justify-center bg-black/90 backdrop-blur-md overflow-y-auto py-6"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          {/* Modal content */}
          <motion.div
            className="relative w-full max-w-sm mx-4 bg-gradient-to-b from-[#1a1f26] to-[#161B22] border border-[#FFC72C]/30 rounded-3xl overflow-hidden my-auto"
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="text-center">
                {/* Icon */}
                <motion.div
                  className="w-14 h-14 mx-auto mb-3 rounded-2xl bg-gradient-to-br from-[#FFC72C] to-[#FF8C00] flex items-center justify-center shadow-lg shadow-[#FFC72C]/30"
                  initial={{ scale: 0, rotate: -10 }}
                  animate={{ scale: 1, rotate: 0 }}
                  transition={{ type: 'spring', damping: 15, stiffness: 200, delay: 0.1 }}
                >
                  <Sparkles className="w-7 h-7 text-[#0d1117]" />
                </motion.div>

                {/* Prestige badge */}
                <motion.div
                  className="inline-flex items-center gap-2 px-4 py-1.5 mb-2 rounded-full bg-[#FFC72C]/10 border border-[#FFC72C]/30"
                ...
                {/* Title */}
                <motion.h1
                  className="text-xl font-bold text-[#E6EDF3] mb-1"
                ...
                {/* Subtitle */}
                <motion.p
                  className="text-sm text-[#8B949E] leading-relaxed mb-4"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                ...
                >
                  {FEATURES.map((feature, index) => (
                    <motion.div
                      key={index}
                      className="flex items-start gap-2 p-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-left"
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.35 + index * 0.05 }}
                    >
                      <div className="w-7 h-7 rounded-lg bg-[#FFC72C]/10 flex items-center justify-center shrink-0">
                        <feature.icon className="w-4 h-4 text-[#FFC72C]" />
                      </div>
                      <div>
                        <span className="text-[11px] font-semibold text-[#E6EDF3] block leading-tight">{t(feature.text)}</span>
                        <span className="text-[9px] text-[#8B949E] leading-tight">{feature.desc}</span>
                      </div>
                    </motion.div>
                  ))}
                </motion.div>
