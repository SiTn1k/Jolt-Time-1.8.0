    ...
    <div className="min-h-full bg-[#0D1117] p-4 pb-20">
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-xl" style={{ fontFamily: "'Exo 2', sans-serif" }}>Реставраційна лабораторія</h1>
            <p className="text-xs text-muted-foreground">Збереження артефактів</p>
          </div>
        </div>
      ...
      <Tabs defaultValue="restoring" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-[#161B22] mb-4">
          <TabsTrigger value="damaged" style={{ fontFamily: "'Exo 2', sans-serif" }}>Пошкоджені</TabsTrigger>
          <TabsTrigger value="restoring" style={{ fontFamily: "'Exo 2', sans-serif" }}>У роботі</TabsTrigger>
          <TabsTrigger value="restored" style={{ fontFamily: "'Exo 2', sans-serif" }}>Готові</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="damaged" className="space-y-3">
          {damaged.length === 0 ? (
            <Card className="border-white/10 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Немає пошкоджених артефактів</p>
            </Card>
          ) : (
              ...
              <motion.div key={artifact.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
                <Card className="border-white/10 p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Badge style={{ backgroundColor: rarityConfig[artifact.rarity].color, color: '#0D1117' }}>{rarityConfig[artifact.rarity].label}</Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <div className="bg-[#0D1117] rounded p-2">
                      <div className="text-xs text-muted-foreground mb-1">Цінність</div>
                      <div className="text-sm" style={{ fontFamily: "'Exo 2', sans-serif", color: '#FFC72C' }}>{artifact.value.toLocaleString()}</div>
                    </div>
                    <div className="bg-[#0D1117] rounded p-2">
                      <div className="text-xs text-muted-foreground mb-1">Престиж</div>
        ...
        <TabsContent value="restoring" className="space-y-3">
          {restoring.length === 0 ? (
            <Card className="border-white/10 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Немає активних реставрацій</p>
              <p className="text-xs text-muted-foreground mt-1">Оберіть пошкоджені артефакти</p>
            </Card>
          ) : (
                  ...
                  <Card className="border-2 p-4" style={{ borderColor: '#FFC72C' }}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        ...
                        {artifact.restoredBy && <p className="text-xs" style={{ color: '#FFC72C' }}>Команда: {artifact.restoredBy}</p>}
                      </div>
                      <Badge style={{ backgroundColor: rarityConfig[artifact.rarity].color, color: '#0D1117' }}>{rarityConfig[artifact.rarity].label}</Badge>
                    </div>
                    ...
                    <div className="flex items-center gap-1 mt-2">
                      ...
                      <span className="text-xs text-muted-foreground">Залишилось {remaining}с</span>
                    </div>
                  </Card>
                </motion.div>
              );
        ...
        <TabsContent value="restored" className="space-y-3">
          {restored.length === 0 ? (
            <Card className="border-white/10 p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Немає завершених реставрацій</p>
            </Card>
          ) : (
